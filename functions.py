...
APPROACHES (Discrete [D] and Continuous [C] Estimators):
    ...
    [D] WEIGHTED SYMBOLIC (Tyson Pond; rework)
...
""" Remember that MI is non-directional; Some efforts to
address this limitation include the use of time-lagged mutual information, in which
mutual information is calculated between a time-lagged version of the source process
and the destination process. This approach remedies the directional deficiency of
mutual information; however, it is still deficient as it incorporates the measurement
of statically shared information between the processes or a common history imposed
by an exogenous force as pointed out in [45]. In this vein, Schreiber proposed a
new measure of information transfer which overcomes some limitations of former 
approaches. Broadly speaking, Schreiber quantified information transfer as a 
KLdivergence amongst conditional probabilities.
...
def jpmd_hist(X: np.array, Y: np.array, bins: int, Z: np.array | None = None):
    '''
...
def entropy_hist(epo1: mne.Epochs, epo2: mne.Epochs):
    '''
...
def mi_hist(epo1: mne.Epochs, epo2: mne.Epochs, vis = False):
    ...
    vis : boolean, optional
        Whether to produce visualisation/plots of entropies and MI. The default is False.
...
def cmi_hist(epo1: mne.Epochs, epo2: mne.Epochs, l = 3):
...
""" QUESTIONS:
        - How many bins/edges?
        - In jpmd_hist: Why does np.histogramdd give marginally different results? (More effective to just use that?)
...
# =============================================================================
# SYMBOLIC ENTROPY, MUTUAL INFORMATION, TRANSFER ENTRTOPY (TYSON POND)
# =============================================================================
...
""" QUESTIONS:
        - This is the most efficient code I could find, there is no point trying to code from scratch. How do I utilise this in my project?    
"""
...
# =============================================================================
# GAUSSIAN COPULA ENTROPY, MUTUAL INFORMATION, (INCE)
# =============================================================================
...
def mi_gc(epo1: mne.Epochs, epo2: mne.Epochs, biascorrect=True):
    ...
    x = copnorm(np.atleast_2d(a)) # instead of shape (501,) --> (1, 501), x.ndim = 2
    y = copnorm(np.atleast_2d(b))
...
""" QUESTIONS:
        - Again, this is the most efficient code I could find (Ince, 2017). How do I utilise this in my project?    
        - How can I rewrite this assuming that I am working with univariate variables (and am I?) - there is a lot of redundant Nvarx, Nvarxy, etc.
"""
...
def entropy_kde(epo1: mne.Epochs, epo2: mne.Epochs, h: float | None = None, kernel: str = 'epanechnikov'):
    ...
    """h ~ .33, but far too insensitive, .33e-5 works, however (is this because it is in the range of EEG values?"""
    
    
    """How do I use a multivariate kernel?
    
        If I want maximum estimate, I can assume independence (p(x,y) = p(x)p(y))
        Since H(X,Y) <= H(X) + H(Y) with equality iff independence.
        
        In this case, dens_XY = dens_X * dens_Y
    
    """
    
    # Now need X-axis to be from lowest value to highest (with bandwidth tolerance to allow final kernel)
    ...
    dens_X = np.exp(kde_X.score_samples(X_plot))
    dens_Y = np.exp(kde_Y.score_samples(Y_plot))
    
    # Normalise
    dens_X /= np.sum(dens_X)
    dens_Y /= np.sum(dens_Y)
    
    dens_XY = dens_X * dens_Y
    dens_XY /= np.sum(dens_XY)
    
    # Calculate Entropies
    entropy_X = -np.sum(dens_X * np.log2(dens_X + np.finfo(float).eps))
    entropy_Y = -np.sum(dens_Y * np.log2(dens_Y + np.finfo(float).eps))
    joint_entropy = -np.sum(dens_XY * np.log2(dens_XY + np.finfo(float).eps))
    ...
    axes[1, 0].twinx().plot(X_plot, dens_X, color='red', label='KDE X')
    axes[1, 0].legend()
    ...
    axes[1, 1].twinx().plot(Y_plot, dens_Y, color='red', label='KDE Y')
    axes[1, 1].legend()
    axes[1, 1].set_xlim(np.min(Y) - h, np.max(Y) + h)
    axes[1, 1].set_xlabel("Amplitude (μV)")
    
    # Scatter Plot
    axes[2, 0].scatter(XY[0], XY[1], alpha=0.5, s = 5)
    ...
    # Joint KDE Plot
    sns.kdeplot(x=XY[0], y=XY[1], fill=True, cmap="Blues", bw_method=optimal_h(1, len(X)), ax=axes[2, 1])
    ...
    # Joint and Marginal Distributions with KDE Overlay
    sns.set()
    ...
    grid.ax_marg_x.twinx().plot(X_plot, dens_X, color='red', label='KDE X')
    
    grid.ax_marg_y.hist(Y, bins=fd_bins, color='blue', alpha=0.7, orientation='horizontal')
    grid.ax_marg_y.twinx().plot(Y_plot, dens_Y, color='red', label='KDE Y')
...
X = epo1.get_data(copy=False)
Y = epo2.get_data(copy=False)
